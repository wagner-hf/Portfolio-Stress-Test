    ...
    (function () {
      ...
      function initStressTest() {
        ...
        const validators = {
          ...
          7: () => {
            STATE.name = $('[data-input="name"]').value.trim();
            STATE.email = $('[data-input="email"]').value.trim();
        ...
        function goToStep(next) {
          ...
          $(`[data-step="${STATE.currentStep}"]`)?.classList.remove('active');
          $(`[data-step="${next}"]`)?.classList.add('active');
          STATE.currentStep = next;

          $$('.step-pip').forEach((pip, idx) => pip.classList.toggle('active', idx < STATE.currentStep));

          if (next === STATE.totalSteps) {
            computeLogic();
            renderResults();

            // Magia para Webflow: Damos 50ms para que pinte el display:block antes de dibujar
            setTimeout(() => {
              drawMatrix();
              drawProjections();
            }, 50);
        ...
        function calculateFreedomNumber() {
          if (STATE.income > 0 && STATE.returnRate > 0) {
            ...
            $('[data-display="mini-freedom-number"]').textContent = formatMoney(STATE.freedomNumber);
          }
        }
        ...
        function initSliders() {
          ASSET_KEYS.forEach(key => {
            ...
            $(`[data-display="value-${key}"]`).textContent = `${s.value}%`;

            s.addEventListener('input', (e) => {
              STATE.allocation[key] = parseFloat(e.target.value);
              $(`[data-display="value-${key}"]`).textContent = `${e.target.value}%`;
        ...
        function initUI() {
          document.addEventListener('click', e => {
            ...
            if (btn) {
              ...
              else if (type.startsWith('beh')) { STATE.behavioral[type] = valStr; }
              else {
            ...
            if (check) {
              ...
              if (check.classList.contains('selected')) STATE.concerns.push(val);
        ...
        async function submitToWebhook() {
          ...
          try {
            ...
            if (res.ok || res.type === 'opaque') { goToStep(8); }
        ...
        function renderResults() {
          $('[data-result="score"]').textContent = STATE.vulnerabilityScore;
          ...
          $('[data-result="criticals"]').textContent = STATE.criticalCount > 0 ? `⚠ ${STATE.criticalCount} Critical Vulnerabilities` : '✅ No Critical Vulnerabilities';
          $('[data-result="stage"]').textContent = STATE.stage;
          $('[data-result="stage-desc"]').textContent = STATE.stageLabel;
          $('[data-result="freedom"]').textContent = formatMoney(STATE.freedomNumber);
          ...
          STATE._charts.proj = new Chart(ctx, {
            ...
            data: {
              ...
              datasets: [
                ...
                { label: 'Potential Path', data: dataFixed, borderColor: '#2ecc71', backgroundColor: 'rgba(46, 204, 113, 0.1)', fill: true, pointRadius: 0 }
              ]
            },
            options: {
              responsive: true,
      ...
      function initApp() {
        ...
        // if (document.getElementById('iv-tickerInput')) initIntrinsicValue(); (De tu código anterior)
      }
